...
test("ZIP path canonicalization and method/flag boundaries reject hostile variants",async()=>{
  const mime={name:"mimetype",text:"application/epub+zip"};
  for(const name of ["/x","C:/x","a\\b","a//b","a/./b","a/../b"])
    await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name,text:"x"}])),e=>e.code==="ZIP_PATH",name);
  await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"x",text:"1"},{name:"x",text:"2"}])),e=>e.code==="ZIP_PATH");
  await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"café",text:"1",flags:0x800},{name:"café",text:"2",flags:0x800}])),e=>e.code==="ZIP_PATH");
  await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"OPS/x/",text:""},{name:"OPS/x",text:"x"}])),e=>e.code==="ZIP_PATH");
  await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"unused",nameBytes:Uint8Array.of(0xff),text:"x",flags:0x800}])),e=>e.code==="ZIP_PATH");
  for(const flags of [1,2,4,16,64])await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"x",text:"x",flags}])),e=>e.code==="ZIP_FLAGS",String(flags));
  await assert.rejects(async()=>gi.parseZip(await makeZip([mime,{name:"x",text:"x",method:12}])),e=>e.code==="ZIP_METHOD");
});

test("descriptor corruption and actual decompression ceilings fail closed",async()=>{
  const bytes=await makeZip([{name:"mimetype",text:"application/epub+zip"},{name:"x",text:"payload ".repeat(200),method:8,descriptor:true}]);
  const parsed=gi.parseZip(bytes),entry=parsed.names.get("x"),descriptor=entry.data+entry.comp;
  bytes[descriptor+4]^=1;
  assert.throws(()=>gi.parseZip(bytes),e=>e.code==="ZIP_DESCRIPTOR");

  const declared=await makeZip([{name:"mimetype",text:"application/epub+zip"},{name:"x",text:"payload ".repeat(200),method:8}]);
  const z1=gi.parseZip(declared),e1=z1.names.get("x");e1.size=1;
  await assert.rejects(()=>gi.readZipEntry(z1,e1,{n:0}),e=>e.code==="ZIP_BOMB");
  const z2=gi.parseZip(await makeZip([{name:"mimetype",text:"application/epub+zip"},{name:"x",text:"payload ".repeat(200),method:8}])),e2=z2.names.get("x");
  await assert.rejects(()=>gi.readZipEntry(z2,e2,{n:gi.GI_LIMIT.total}),e=>e.code==="ZIP_BOMB");
});
...
test("href and lexical DOCTYPE boundaries are exact",()=>{
  ...
  gi.checkXmlDoctype("<!DOCTYPE  \t\r\n html \t><html/>",true);
  gi.checkXmlDoctype("<![CDATA[<!DOCTYPE bad>]]><html/>",true);
  assert.throws(()=>gi.checkXmlDoctype("<!DOCTYPE html PUBLIC 'x'><html/>",true),e=>e.code==="XML_DOCTYPE");
  assert.throws(()=>gi.checkXmlDoctype("<!DOCTYPE html><html/>",false),e=>e.code==="XML_DOCTYPE");
...
test("validation, stale commit, restore clamp, and serialized writes are transactional",async()=>{
  ...
  assert.equal(gi.genericRestoreSeg(1,2),1);
  for(const bad of [-1,2,1.5,"1",null])assert.equal(gi.genericRestoreSeg(bad,2),0);
  let token=2,commits=0;
  assert.equal(gi.commitGenericDoc(doc,"fixture",1,()=>token,()=>{commits++;}),false);
  assert.equal(commits,0);
  assert.throws(()=>gi.commitGenericDoc({...doc,title:""},"fixture",2,()=>token,()=>{commits++;}),e=>e.code==="CONTRACT");
  assert.equal(commits,0);
